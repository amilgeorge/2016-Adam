...
OFFSETS = [(fnc.POLICY_OFFSET,0)]
#OFFSETS = [(fnc.POLICY_DEFAULT_ZERO,0)]
offset_string = "-".join(str(x) for p,x in OFFSETS)
dbname = imdb.IMDB_CUSTOM_MASK_DAVIS2016
lr = 1e-2

RUN_ID = "s480p-osvosmask-{}-O{}-osvosold-reg1e-4-mo<1e-2>-de-scale1.3-4".format(dbname,offset_string)
...
def train():
    ...
    SAVE_STEP_SIZE = 3000
    custom_mask_folder = os.path.join("..","Results","OSVOS")
    ...
    with tf.Graph().as_default():
        ...
        with tf.Session(config=tf.ConfigProto(log_device_placement=False)) as sess:
            ...
            inputProvider.db.set_policy(imdb.CustomMaskDB2016.POLICY_CM, None)
            ...
            while global_step_var.eval() <= max_iters:
                ...
                if step % SAVE_STEP_SIZE == 0:
                    ...
                    test_segnet2.test_network(sess,net_dict,test_out_dir,pfc,prev_mask_dir=custom_mask_folder)
