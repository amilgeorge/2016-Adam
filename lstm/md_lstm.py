...
def multi_dimensional_rnn_while_loop(rnn_size, input_data, sh, dims=None, scope_n="layer1"):
    ...
    with tf.variable_scope("MultiDimensionalLSTMCell-" + scope_n):
        ...
        shape1 = tf.stack([batch_size,  h, w, features])
        #x = tf.reshape(input_data, [batch_size, h, w, features])
        x = tf.reshape(input_data, shape1)
        ...
        shape2 = tf.stack([batch_size,rnn_size])
        states_ta = states_ta.write(h * w, LSTMStateTuple(tf.zeros(shape2, tf.float32),
                                                          tf.zeros(shape2, tf.float32)))
