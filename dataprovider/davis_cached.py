...
class DataAccessHelper(object):
    ...
    def __init__(self,resize=[480,854]):
        '''
        Constructor
        '''
        self.image_set = '2017'
    ...
    def cache_exists(self):
        index_dict_file = os.path.join(self.cache_dir, 'name_idx_dict.p')
    ...
    def read_label(self,label_path,object_id = None):
        ...
        idx = self.get_cached_idx(seq, frame_no)
        label = (self.db.labels[idx, :, :] == object_id)
        return label
...
if __name__ == '__main__':
    ...
    row_s= []
    col_s = []
    for seq in helper.train_sequence_list()+helper.test_sequence_list():
        img_path = helper.image_path(seq,0)
        img_shape = helper.image_shape(img_path)
        print(img_path)
        row_s.append(img_shape[0])
        col_s.append(img_shape[1])
