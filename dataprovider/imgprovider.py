...
class InputProvider:
    ...
    def __init__(self, offsets = None,is_dummy = False,db_type = 0):
        ...
        self.offsets = offsets#list(range(1,7))#[10]

        if (db_type ==0):
            self.db = self.createDB0(self.train_set_info,name = train_db_name)
            self.validation_db = self.createDB0(self.val_set_info,name = val_db_name)
...
#                     plt.colorbar()
    ...
    def prepare_temp_files_0(self,data_set_info,out_dir):
        ...
        for i in range(num_images):
                ...
                # prevMaskFile = self.getPrevMaskFile(labelFile)
                image = self.read_image0(imageFile)
                if (np.count_nonzero(image[:, :, 3]) == 0):
                    print("skipping image all zeros: {}".format(imageFile))
                    continue
                label = self.davis.read_label(labelFile, self.resize)
                outfile = os.path.join(out_dir,'file-{}.npz'.format(i))
                np.savez(outfile,image=image,label=label)
    ...
    def createDB0(self, data_set_info, name=''):
        print("Creating DB0 ")
        ...
        if not (os.path.isfile(images_cached_file) and os.path.isfile(labels_cached_file)):
            ...
            self.prepare_temp_files_0(data_set_info, tmp_files_dir)
...
def test_gen_samples():
    ...
    provider = InputProvider(offsets =[1] , is_dummy=False,db_type = 0)
    num=0
    dir = 'davis_samp_0/'
...
if __name__ == '__main__':
    provider = InputProvider(resize=[IMAGE_HEIGHT,IMAGE_WIDTH],is_coarse=False,is_dummy=True)
    batch_size = 4
    num=0
