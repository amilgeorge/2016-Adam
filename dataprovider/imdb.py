...
IMDB_CUSTOM_SEQ_DAVIS2016 = 'custommaskseqdavis2016'
...
class CUSTOMSEQDB2016:

    IMDB_NAME = IMDB_CUSTOM_SEQ_DAVIS2016
    ...
    def set_mask_folder(self,folder_path):
        ...
        if self.policy == CUSTOMSEQDB2016.POLICY_SELECT_CM:
            if self.stats is not None:
                self.stats_hist.append(self.stats)

            self.stats = CUSTOMSEQDB2016.StatsSelectCM(folder_path)
            print(self.disp_stats())
    ...
    def custom_label_path(self,seq,frame_no):
        return os.path.join(self.mask_folder,'{0:05}.png'.format(frame_no))
...
def get_cust_seq_davis_2016_imdb(seq):
    db = CUSTOMSEQDB2016(seq)
    return db
...
def get_imdb(dbname):
    ...
    elif dbname.startswith(IMDB_CUSTOM_SEQ_DAVIS2016):
        p = re.split(',', dbname)
        return get_cust_seq_davis_2016_imdb(p[1])
