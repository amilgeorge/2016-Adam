...
def crop_features(feature, out_size):
    ...
    return tf.reshape(slice_input, [tf.cast(up_size[0],tf.int32), out_size[1], out_size[2], int(feature.get_shape()[3])])

def osvos_net(inputs, scope='osvos'):
    ...
    with tf.variable_scope('osvos', 'osvos', [inputs]) as sc:
    ...
    with tf.variable_scope('osvos', 'osvos', [inputs]) as sc:
        ...
        with slim.arg_scope([slim.conv2d, slim.max_pool2d],
            ...
            with slim.arg_scope([slim.conv2d],
                ...
                concat_side = tf.concat([side_2_f, side_3_f, side_4_f, side_5_f], axis=3)
                utils.collect_named_outputs(end_points_collection, 'osvos/concat_side', concat_side)
...
def preprocess_img_ch7(image):
    ...
    """
    in_ = image[:, :, :,[2,1,0,3,6,5,4]]
    in_ = np.subtract(in_, np.array((104.00699, 116.66877, 122.67892,128.0,104.00699, 116.66877, 122.67892), dtype=np.float32))
...
def load_ckpt_ch7(ckpt_path):
    ...
    for v in var_to_shape_map:
        if "conv" in v and not (v == "osvos/conv1/conv1_1/weights"):
            print(v)
...
def load_vgg_imagenet(ckpt_path):
    ...
    vars_corresp = dict()
    for v in var_to_shape_map:
        if "conv" in v and not (v == "vgg_16/conv1/conv1_1/weights"):
            print(v)
...
def load_first_conv(ckpt_path):
    reader = tf.train.NewCheckpointReader(ckpt_path)
    vars_corresp = dict()
    val = reader.get_tensor('vgg_16/conv1/conv1_1/weights')
    print(val.shape)
    vars_corresp['osvos/conv1/conv1_1/weights'] = prepare_conv1_w(val)
    return slim.assign_from_values_fn(vars_corresp)

def load_first_convV2(ckpt_path):
    reader = tf.train.NewCheckpointReader(ckpt_path)
    vars_corresp = dict()
    val = reader.get_tensor('osvos/conv1/conv1_1/weights')
    print(val.shape)
    vars_corresp['osvos/conv1/conv1_1/weights'] = prepare_conv1_w(val)
    return slim.assign_from_values_fn(vars_corresp)
...
def class_balanced_cross_entropy_loss(output, label):
    ...
    final_loss = num_labels_neg / num_total * loss_pos + tf.maximum(num_labels_pos,1.0) / num_total * loss_neg
...
def prepare_conv1_w(val):
    prev_mask_filt_array = get_init_prev_mask_filters(val)
    print(val.shape)
    print(prev_mask_filt_array.shape)
...
def load_caffe_weights2(weights_path):
    ...
    vars_corresp = dict()
    vars_corresp['osvos/conv1/conv1_1/weights'] = prepare_conv1_w(osvos_weights['conv1_1_w'])
